...
private fun printItem (item : Card){
    when(item){
        is Card.Dog -> print(String.format("${String(Character.toChars(Animal.DOG.unicode))}%02d",item.num))
        is Card.Cat -> print(String.format("${String(Character.toChars(Animal.CAT.unicode))}%02d",item.num))
        is Card.Cow -> print(String.format("${String(Character.toChars(Animal.COW.unicode))}%02d",item.num))
    }
}